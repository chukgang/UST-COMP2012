    ...
    while(true){
        ...
        switch(option)
        {
            ...
            case 2:{
                ...
                if(timeManager.addPerson(personName)){
                	cout << personName << " has been added." << endl;	
				}else{
					cout << personName << " cannot be added due to a person name conflict." << endl;
				}
                break;
            }
            case 3:{
                ...
                if(timeManager.addActivityForPerson(personName, Activity(activityName, Timeslot(activityStartTime, activityEndTime)))){
                	cout << activityName << " has been added." << endl;
				}
                else{
                	cout << activityName << " cannot be added due to an activity name/time conflict." << endl;
				} 
                break;
            }
            case 4:{
            	...
            	string personName = getStringInput("What is the person's name?");
                if(timeManager.removePerson(personName)){
                	cout << personName << " has been removed." << endl;
				} 
                else{
                	cout << "Person name does not exist in the database." << endl;
				} 
                break;
            }
            case 5:{
            	...
            	string activityName = getStringInput("What is the activity's name?");
                if(timeManager.removeActivityForPerson(personName, activityName)){
                	cout << activityName << " has been removed." << endl;
				} 
                else{
                	cout << activityName << " cannot be removed - activity not found." << endl;
				} 
                break;
            }
            case 6:{
            	...
            	if(timeslot == NULL){
            		cout << "No common timeslot is found." << endl;
				} 
            	else{
